...
let
  # Single source of truth for where wallpapers live. Every script below
  # honours a WALLPAPER_DIR override from the environment so the same
  # binaries can be pointed at another directory ad hoc.
  ...
  thumbWorkerScript = pkgs.writeShellScript "wallpaper-thumb-one" ''
    set -uo pipefail
  ...
  wallpaperThumbsScript = pkgs.writeShellScriptBin "wallpaper-thumbs" ''
    #!/usr/bin/env bash
    set -uo pipefail
...
in
{
  ...
  # Warm the thumbnail cache at session start so the first Mod+Shift+B of
  # the session opens instantly instead of blocking on a full render pass
  ...
  systemd.user.services.wallpaper-thumbs = {
    ...
    serviceConfig = {
      Type = "oneshot";
      ExecStart = "${wallpaperThumbsScript}/bin/wallpaper-thumbs sync";
      StandardOutput = "null";
      Nice = 10;
