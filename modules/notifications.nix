...
#
# Visual config lives in ~/.config/mako/config (outside Nix, same as the
# rest of this setup); this module just makes sure the binaries exist and
# runs the sound watcher as a systemd user service.
...

{
  ...
  # the existing `makoctl reload` call in dark-mode.d/light-mode.d's
  # 20-reload-bar.sh — no changes needed there.
