#!/usr/bin/env bash
# darkman light-mode hook: re-run wallust on the current wallpaper with a
# light palette, so everything wallust drives — the Quickshell shell (watches
...
LIGHT_PALETTE="softlight"   # matches the default in wallust.toml
...
wallust run "$wallpaper_path" -p "$LIGHT_PALETTE" -s 2>/dev/null
